...
public class TelemedController {
    ...
    @Operation(summary = "(환자) 1) 구음장애 환자의 음성을 텍스트로 변환하여 제공하는 API", description = "환자의 음성을 녹음하여 전송합니다. db에 저장하지 않고 그냥 반환만 합니다.")
    ...
    public ResponseEntity<ApiResponse<Object>> speechPatient(@AuthenticationPrincipal CustomUserDetails userDetails,
                                                             @PathVariable String roomId,
                                                             @RequestPart("file") MultipartFile file)throws Exception{
        return ResponseEntity.ok(ApiResponse.from(telemedChatService.transcribe(userDetails.getId(), roomId, file)));
    }

    @Operation(summary = "(환자) 2) 변환된 텍스트를 입력하여 openAI를 통해 3가지 선택지를 제공하는 API", description = "변환된 텍스트를 입력해주세요")
    @PostMapping("/{roomId}/speech-text")
    public ResponseEntity<ApiResponse<Object>> speechText(@AuthenticationPrincipal CustomUserDetails userDetails,
                                                          @PathVariable String roomId,
                                                          @RequestBody SendSpeechRequest request){
        return ResponseEntity.ok(ApiResponse.from(telemedChatService.getSpeechText(userDetails.getId(), roomId, request)));
    }

    @Operation(summary = "(환자) 3) 선택한 3가지 중 하나를 선택해 의사에게 전송하는 API", description = "한 가지를 의사에게 전달하고 db에 저장합니다.")
    ...
    public ResponseEntity<ApiResponse<Object>> historyDetail(@AuthenticationPrincipal CustomUserDetails userDetails,
                                                             @PathVariable String roomId){
